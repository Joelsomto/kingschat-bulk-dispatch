...
//         // Pre-calculate retries for this batch
//         setRetryCounts(prev => {
...
function App() {
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      ...
      ["accessToken", "refreshToken", "expiresIn"].forEach((key) => {
        const input = document.createElement("input");
        input.type = "hidden";
        input.name = key;
  ...
  useEffect(() => {
    const verify = async () => {
      ...
      try {
        const sessionData = JSON.parse(session);
        const res = await fetch("https://kingslist.pro/app/default/api/verify_session.php", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ accessToken: sessionData.accessToken }),
        });
  ...
  const updateDispatchStatus = useCallback(async (dmsg_id) => {
    try {
      const { success, failed } = progressRef.current;
      const totalAttempts = Object.values(retryCounts).reduce((a, b) => a + b, 0);
      const status = failed > 0 ? 1 : 2;

      const res = await fetch("https://kingslist.pro/app/default/api/updateDispatchCount.php", {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          dmsg_id,
          dispatch_count: processedMessages.size,
          attempts: totalAttempts,
          status,
        }),
      });
  ...
  const handleDispatch = useCallback(async (dmsg_id) => {
    ...
    try {
      const rawData = await fetchDispatchBatch(dmsg_id);
      let messages = prepareMessagesForDispatch(rawData);

      messages = messages.map(msg => ({
        ...msg,
        body: msg.body.replace(/<kc_username>/g, msg.username).replace(/<fullname>/g, msg.fullname),
      }));
      ...
      while (pending.length > 0) {
        ...
        for (const msg of pending) {
          if (processedMessages.has(msg.kc_id)) continue;
          localRetries[msg.kc_id] = (localRetries[msg.kc_id] || 0) + 1;

          try {
            await new Promise(res => setTimeout(res, MESSAGE_DELAY_MS));
            const result = await sendMessage(accessToken, msg.kc_id, msg.body);
            ...
            console.warn(`Send failed for ${msg.kc_id}:`, e.message);
          }

          if (localRetries[msg.kc_id] < MAX_RETRY_ATTEMPTS) {
            nextRound.push(msg);
          } else {
            updateProgress(prev => ({
              ...prev,
      ...
      const status = await updateDispatchStatus(dmsg_id);
      if (!status.success) throw new Error("Final status update failed");

      sessionStorage.setItem(`dispatch_status_${dmsg_id}`, "completed");
      sessionStorage.setItem(`dispatch_analytics_${dmsg_id}`, JSON.stringify({
        success: progressRef.current.success,
        failed: progressRef.current.failed,
        retries: Object.values(localRetries).filter(c => c > 1).length,
      }));

      const url = new URL(window.location.href);
      url.searchParams.set("start_dispatch", "2");
  ...
  const ProgressBar = () => {
    ...
    const percent = (progress.current / progress.total) * 100;
    return (
      <div style={{ marginTop: 10, background: "#e0e0e0", borderRadius: 8, height: 20 }}>
        <div style={{
          width: `${percent}%`,
          background: percent === 100 ? "#28a745" : "#007bff",
          height: "100%",
  ...
  const DispatchAnalytics = () => {
    ...
    return (
      <div style={{ marginTop: 20, padding: 10, border: "1px solid #ccc", borderRadius: 8 }}>
        {progress.current >= progress.total && (
          <div style={{ color: "#28a745", fontWeight: "bold", marginBottom: 10 }}>
            {progress.failed > 0 ? "Dispatch Completed with Some Failures" : "Dispatch Successfully Completed"}
          </div>
        )}
        <h4>Dispatch Summary</h4>
        <p><strong>Total:</strong> {progress.total}</p>
        ...
        <p style={{ color: "#dc3545" }}><strong>Failed:</strong> {progress.failed}</p>
        <p style={{ color: "#ffc107" }}><strong>Retried:</strong> {
          Object.values(retryCounts).filter(c => c > 1).length
        }</p>
      </div>
    );
  };
  ...
  return (
    <div style={{ padding: 20 }}>
      {!isLoggedIn ? (
        <button onClick={handleLogin}>Login to KingsChat</button>
      ) : (
        <>
          {ProgressBar()}
          {DispatchAnalytics()}
